...
# Convert to snake_case, make sure there's no unneccesary symbols, remove characters that are not numbers, letters, or underscores)
...
def clean_column_names(df: pd.DataFrame, inplace: bool = False) -> pd.DataFrame:
	...
	seen = set()
	unique = []
	# Make column names unique; Author modifcations include variable renaming (replacing 'i' with 'count' for clarity and code-style preference)
	for base in new:
		candidate = base
		count = 1
